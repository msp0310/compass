...
async function login(request: APIRequestContext): Promise<AuthSession> {
  ...
  if (!sessionCookie || !csrfCookie) throw new Error("認証Cookieを取得できませんでした。");
  return {
...
test("ZstandardでAPIレスポンスを圧縮し、復元できる", async ({ request }) => {
  ...
  const response = await getRawApiResponse("/api/projects/site-renewal/schedule", session.cookieHeader);
...
test("日報の作業明細を案件実績へ反映し、削除時に取り消せる", async ({ request }) => {
  ...
  expect((await commentResponse.json()).comments).toContainEqual(
  ...
  const reminders = (await reminderResponse.json()) as Array<{ id: string }>;
  expect(reminders).toHaveLength(1);
  expect((await request.get("/api/daily-reports/reminders", { headers })).ok()).toBe(true);
  expect(
    (
      await request.post(`/api/daily-reports/reminders/${reminders[0]?.id}/read`, { headers })
    ).status(),
  ).toBe(204);
...
test("プロジェクトスケジュールを同一内容で差分保存できる", async ({ request }) => {
  ...
  for (let attempt = 0; attempt < 3; attempt += 1) {
    ...
    schedule = await (await request.get("/api/projects/site-renewal/schedule", { headers })).json();
  }
  expect(saveResponse?.ok()).toBe(true);
  if (!saveResponse) throw new Error("案件保存レスポンスを取得できませんでした。");
...
test("プロジェクトNo.を案件情報として保存・取得できる", async ({ request }) => {
  ...
  const original = await (await request.get(endpoint, { headers })).json();
