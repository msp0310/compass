...
test("新規日報は指定日の下書きと最初のタスク実績を作る", () => {
  ...
  expect(canSubmitDailyReport(report)).toBe(false);
  report.summary = "API実装を進めました。";
  report.entries[0]!.summary = "APIの入力検証を追加";
  report.entries[0]!.progress = 35;
  expect(canSubmitDailyReport(report)).toBe(true);
});

test("タスク実績サマリーは作業時間と予定工数超過を算出する", () => {
  const entries = [
    {
  ...
  expect(getDailyReportAverageProgress(entries)).toBe(60);
});
