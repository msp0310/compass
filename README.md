...
ブラウザで [http://127.0.0.1:8080/](http://127.0.0.1:8080/) を開きます。Development環境では、ローカル開発用の初期アカウントとサンプル案件が作成されます。SQLiteのデータは `mirai-data` ボリュームに保存されます。

本番運用では `ASPNETCORE_ENVIRONMENT=Production` を指定し、管理者アカウントの作成経路と秘密情報を別途用意してください。
