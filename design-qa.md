...
- Fonts and typography: existing Inter/Noto Sans JP stack retained; parent rows are visibly heavier and leaf rows remain lighter. Truncation and 34 px row rhythm remain intact.
- Spacing and layout rhythm: compact 12 px depth steps, continuous ancestor rails, elbow connectors, and existing row height align with the selected target without expanding the table.
...
- Root collapse reduced visible task rows from 28 to 1; expanding restored all 28 rows.
