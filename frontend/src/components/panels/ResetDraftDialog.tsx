...
export function ResetDraftDialog({
  ...
  const hiddenConfigCount = Math.max(
    configReview.rows.length - configPreviewRows.length,
    0,
  );
  const projectLabels = [
    ...new Set(
          ...
          <section className="save-review-section" aria-label="破棄される設定差分">
            ...
            <div className="save-review-list">
              {configPreviewRows.map((row) => (
                <article
                  className={`change-review-row config ${row.category}`}
                  key={row.id}
                >
