...
export function SummaryStrip({
  ...
  const totalEffort = actionableTasks.reduce(
    (sum, task) =>
      sum +
      (task.effortHours ??
        getWorkingDays(task.start, task.end, calendar, calendarAware) * 8),
    0,
  );
    ...
    <section className="status-dashboard" aria-label="プロジェクト概要">
      <div className="summary-strip">
        ...
        <SummaryCard
          ...
          detail={formatDateWithWeekday(
            openMilestones[0]?.start ?? project.nextMilestone.date,
          )}
        />
        <SummaryCard
          label="完了率"
      ...
      <div className="dashboard-grid">
        ...
        <section className="dashboard-panel risk-panel">
          <PanelHeader
            title="要確認"
            detail={`${delayedTasks.length + blockedTasks.length}件`}
          />
          <div className="risk-list">
            {[...delayedTasks, ...blockedTasks]
              .filter(
                (task, index, self) =>
                  self.findIndex((item) => item.id === task.id) === index,
              )
              .slice(0, 5)
              .map((task) => (
                <button
          ...
          <div className="health-list">
            ...
            {hiddenHealthIssueCount > 0 ? (
              ...
              >
                {showAllHealthIssues
                  ? "主要5件に戻す"
                  : `残り${hiddenHealthIssueCount}件を表示`}
              </button>
            ) : null}
