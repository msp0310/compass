...
export function ResourcePanel({
  ...
  const selectedWeekLabel = selectedCell
    ? weeks.find((week) => week.key === selectedCell.cell.week)?.label ??
      selectedCell.cell.week
    : "";
              ...
              <div className="resource-settings-popover">
                ...
                <label className="check-row">
                  <input
                    checked={showHours}
                    onChange={(event) =>
                      updateDisplaySettings({ showHours: event.target.checked })
                    }
                    type="checkbox"
                  />
                  工数を表示
                ...
                <label className="check-row">
                  <input
                    checked={compact}
                    onChange={(event) =>
                      updateDisplaySettings({ compact: event.target.checked })
                    }
                    type="checkbox"
                  />
                  コンパクト表示
...
function ResourceDrilldown({
  ...
  const suggestions = buildResourceAdjustmentSuggestions(
    cell,
    currentMemberId,
    resourceRows,
  );
    ...
    <section className="resource-drilldown" aria-label="リソース内訳">
      <div className="resource-drilldown-heading">
        <div>
          ...
          <span>
            {weekLabel} / {cell.hours}h / {cell.percent}% / 枠
            {cell.capacityHours}h
          </span>
        </div>
        <button
          aria-label="工数内訳を閉じる"
            ...
            {suggestions.map((suggestion) => (
              ...
              >
                ...
                <div className="resource-suggestion-impact">
                  <strong>
                    -{formatResourceHours(suggestion.reliefHours)}
                  </strong>
                  <span>
                  ...
                  {scope === "project" ? (
                    <>
                      <button
                        className="subtle-action"
                        onClick={() =>
                          onMoveTask(suggestion.contribution.taskId, -1)
                        }
                        type="button"
                      >
                        1日前
        ...
        {cell.contributions.map((contribution) => (
          ...
          >
            <div>
              ...
              <span>
                {formatShortDate(contribution.start)} -{" "}
                {formatShortDate(contribution.end)} /{" "}
  ...
  return cell.contributions
    .map((contribution) => {
      ...
      const nextPercent =
        cell.capacityHours > 0
          ? Math.round((nextHours / cell.capacityHours) * 100)
          : 0;
      return {
        contribution,
        nextHours,
    ...
    .sort(
      (a, b) =>
        ...
        a.contribution.start.localeCompare(b.contribution.start),
    )
    .slice(0, 3);
}
...
function buildResourceShareCandidates(
  ...
  const currentNextPercent = calculateResourcePercent(
    currentNextHours,
    cell.capacityHours,
  );
  return resourceRows
    ...
    .map((row) => {
      ...
      const targetCapacityHours =
        targetCell?.capacityHours ?? Math.round(row.member.capacityHours);
      const targetCurrentPercent = calculateResourcePercent(
        targetCurrentHours,
        targetCapacityHours,
      );
      const targetNextHours = targetCurrentHours + sharedHours;
      const targetNextPercent = calculateResourcePercent(
        targetNextHours,
        targetCapacityHours,
      );
      return {
