...
export function CalendarPanel({
  ...
  const [visibleMonth, setVisibleMonth] = useState(() =>
    toDateKey(new Date(parseDate(project.rangeStart).getFullYear(), parseDate(project.rangeStart).getMonth(), 1)),
  );
  ...
  const workingDays = useMemo(
    () =>
      Array.from({ length: daysInclusive(toDateKey(monthStart), toDateKey(monthEnd)) }, (_, index) =>
  ...
  const selectedHoliday = calendar.holidays.find(
  ...
  function getVisibleEvents(dateKey: string) {
    return tasks
      ...
      .filter(
        (task) =>
          task.type === "milestone" || task.start === dateKey || task.end === dateKey,
      )
      .sort(
        (a, b) =>
          getCalendarEventPriority(a, dateKey) - getCalendarEventPriority(b, dateKey) ||
