    ...
    <section className="milestone-panel" aria-label="マイルストーン管理">
      ...
      <div className="milestone-layout">
        ...
        <aside className="milestone-create">
          ...
          <button className="primary-button milestone-add-button" onClick={createMilestone} type="button">
            <PlusIcon />
            追加
          </button>
        </aside>
    ...
    <article className={`milestone-card ${milestone.status} ${readiness.tone}`}>
      ...
      <div className="milestone-card-main">
        <div className="milestone-card-title">
          <input
            ...
            onChange={(event) =>
              onUpdateTask(milestone.id, { title: event.target.value })
            }
          ...
          <div className="milestone-gate-list" aria-label="判定材料">
            {readiness.dependencies.map((dependency) => (
              ...
              >
                ...
                <small>
                  {formatShortDate(dependency.start)} -{" "}
                  {formatShortDate(dependency.end)} / {statusLabels[dependency.status]} /{" "}
                  {dependency.progress}%
                </small>
              </button>
            ))}
      ...
      <div className="milestone-controls">
        <label>
          ...
          <input
            onChange={(event) =>
              onUpdateTask(milestone.id, { start: event.target.value })
            }
            onInput={(event) =>
              onUpdateTask(milestone.id, { start: event.currentTarget.value })
            }
            type="date"
...
function getMilestoneReadiness(milestone: ScheduleTask, tasks: ScheduleTask[]) {
  ...
  const scheduleConflictCount = dependencies.filter(
    (task) => task.end > milestone.start,
  ).length;
