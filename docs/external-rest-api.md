...
Miraiの外部REST APIは、社内プロジェクト管理、集計基盤、バッチ、将来のMCPサーバーから案件・タスク・実績を安全に連携するための境界です。
...
# 設定するハッシュの生成
printf '%s' "$MIRAI_EXTERNAL_API_KEY" | shasum -a 256
```
...
開発環境では`mirai-local-external-api-key`を利用できます。このキーは`Development`環境だけで有効です。
...
```bash
curl -X PATCH \
  -H "X-Mirai-Api-Key: $MIRAI_EXTERNAL_API_KEY" \
